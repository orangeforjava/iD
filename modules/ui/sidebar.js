...
import SidebarShell from './vue/SidebarShell.vue';
...
export function uiSidebar(context) {
  ...
  var shellState = {
    setRefs: function(refs) {
      shellState.refs = refs;
    },
    refs: null
  };
  var render = mountVueComponent(SidebarShell, context, { state: shellState });

  function ensureRefs(selection) {
    if (shellState.refs) return;

    var resizer = selection.selectAll('.sidebar-resizer')
      .data([0])
      .enter()
      .append('div')
      .attr('class', 'sidebar-resizer')
      .merge(selection.selectAll('.sidebar-resizer'));

    var featureList = selection.selectAll('.feature-list-pane')
      .data([0])
      .enter()
      .append('div')
      .attr('class', 'feature-list-pane')
      .merge(selection.selectAll('.feature-list-pane'));

    var inspectorWrap = selection.selectAll('.inspector-wrap')
      .data([0])
      .enter()
      .append('div')
      .attr('class', 'inspector-hidden inspector-wrap')
      .merge(selection.selectAll('.inspector-wrap'));

    var currentHost = selection.selectAll('.sidebar-current-host')
      .data([0])
      .enter()
      .append('div')
      .attr('class', 'sidebar-current-host')
      .merge(selection.selectAll('.sidebar-current-host'));

    shellState.refs = {
      resizer: resizer.node(),
      featureList: featureList.node(),
      inspectorWrap: inspectorWrap.node(),
      currentHost: currentHost.node()
    };
  }
  ...
  function sidebar(selection) {
    ...
    var root = selection;
    var resizer = d3_select(shellState.refs.resizer);
    var featureListWrap = d3_select(shellState.refs.featureList).call(uiFeatureList(context));
    var inspectorWrap = d3_select(shellState.refs.inspectorWrap);
    var currentHost = d3_select(shellState.refs.currentHost);
    ...
    resizer.on(_pointerPrefix + 'down.sidebar-resizer', pointerdown);
    ...
    function pointerdown(d3_event) {
      ...
      sidebarWidth = root.node().getBoundingClientRect().width;
      containerWidth = container.node().getBoundingClientRect().width;
      var widthPct = (sidebarWidth / containerWidth) * 100;
      root.style('width', widthPct + '%').style('max-width', '85%');
      resizer.classed('dragging', true);

      d3_select(window)
        .on('touchmove.sidebar-resizer', function(ev) { ev.preventDefault(); }, { passive: false })
    ...
    function pointermove(d3_event) {
      ...
      var isCollapsed = root.classed('collapsed');
      var shouldCollapse = sidebarWidth < minWidth;
      root.classed('collapsed', shouldCollapse);

      if (shouldCollapse) {
        if (!isCollapsed) {
          root.style(xMarginProperty, '-400px').style('width', '400px');
          context.ui().onResize([(sidebarWidth - dx) * scaleX, 0]);
        }
      } else {
        var widthPct = (sidebarWidth / containerWidth) * 100;
        root.style(xMarginProperty, null).style('width', widthPct + '%');
        if (isCollapsed) {
    ...
    var hoverModeSelect = function(targets) {
      ...
      if (context.selectedIDs().length > 1 && targets && targets.length) {
        ...
        if (!elements.empty()) elements.classed('hover', true);
      }
    };
    ...
    function hover(targets) {
      ...
      if (datum && datum.__featurehash__) {
        ...
        root.selectAll('.sidebar-component').classed('inspector-hover', true);
      } else if (datum instanceof osmNote) {
        if (context.mode().id === 'drag-note') return;
        _wasNote = true;
        var osm = services.osm;
        if (osm) datum = osm.getNote(datum.id);
        sidebar.show(noteEditor.note(datum));
        root.selectAll('.sidebar-component').classed('inspector-hover', true);
      } else if (datum instanceof QAItem) {
        _wasQaItem = true;
        var errService = services[datum.service];
        if (errService) datum = errService.getError(datum.id);
        var errEditor = datum.service === 'keepRight' ? keepRightEditor : osmoseEditor;
        context.container().selectAll('.qaItem.' + datum.service).classed('hover', function(d) { return d.id === datum.id; });
    ...
    sidebar.select = function(ids, newFeature) {
      ...
      if (ids && ids.length) {
        ...
        if (entity && newFeature && root.classed('collapsed')) {
    ...
    sidebar.show = function(component, element) {
      ...
      _current = currentHost.append('div').attr('class', 'sidebar-component').call(component, element);
    };
    ...
    sidebar.expand = function(moveMap) {
      if (root.classed('collapsed')) sidebar.toggle(moveMap);
    };
    ...
    sidebar.toggle = function(moveMap) {
      ...
      var startMargin = isCollapsing ? 0 : -sidebarWidth;
      var endMargin = isCollapsing ? -sidebarWidth : 0;
      let lastMargin = startMargin;

      if (!isCollapsing) {
        root.classed('collapsed', isCollapsing);
      }
    ...
    resizer.on('dblclick', function(d3_event) {
      d3_event.preventDefault();
      if (d3_event.sourceEvent) d3_event.sourceEvent.preventDefault();
      sidebar.toggle();
    });

    context.map().on('crossEditableZoom.sidebar', function(within) {
      if (!within && !root.select('.inspector-hover').empty()) {
        hover([]);
      }
    });
  }
  ...
  sidebar.unmount = render.unmount;
